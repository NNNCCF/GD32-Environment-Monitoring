...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	gpio_bit_write(OLED_PORT, OLED_SCL_PIN, (bit_status)BitValue);
	
	/*GD32F103主频108MHz，软件I2C需要适量延时以满足时序要求*/
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	gpio_bit_write(OLED_PORT, OLED_SDA_PIN, (bit_status)BitValue);
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏输出模式*/
	rcu_periph_clock_enable(RCU_GPIOB);
	
	gpio_init(OLED_PORT, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, OLED_SCL_PIN);
	gpio_init(OLED_PORT, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, OLED_SDA_PIN);
